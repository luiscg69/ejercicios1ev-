...
public class AdivinaNumero {
	...
	public static void main(String[] args) {
		 ...
		 * Siendo N otro número aleatorio que genere el ordenador mayor o igual a 1000 y
		 * menor o igual a 100.000 El ordenador nos pregunta que nuemero creemos que fue
		 * el que penso entre 1 y N Introducidos el numero que creeemos que penso y nos
		 * dira si es menor o menor al numero N o si se ha acertado Nos deja meter
		 * numeros hasta que acertemos el valor del número
		 * 
		 */
		...
		do {
			...
			jugar = input.nextLine();// jugar toma el valor introduccio por el usuario por la consola, recordemos que
										// jugar es una String

			// Si la respuesta no es igual a si y no es igual a no entonces la consola nos
			// dice que la respuest es incorrecta
		...
		// preguntar den tro de un bucle vamos a ejecutar el juego

		// las sentencias dentro del bucle while se ejecutarn mientras jugar sea igual a
		// si

		while (jugar.equals("si")) {
			int max; // definimos una variable entera que será el primer numero aleatorio entre 1000
						// y 100.000

			max = 1000 + r.nextInt(99001); /*
			...
			int n; // definimos una variable entera n que será el numero aleatorio entre 1 y el
					// numero aletorio max calculado anteriormente

			n = 1 + r.nextInt(max); /*
									 * como el Ramdon con nextInt(numero) va desde 0 al numero menos 1, el minimo
									 * que necesitamos es 1 y el maximo el max calculado entoces n sera 1 más un
									 * numero aletatorio entre 0 y el numero maximo menos 1
									 */
			int nu; // declaramos una varible entera que será el número que va ser el número que
					// introduzcamos mediante la clase Scanner
			...
			do {
				System.out.printf("¿Qué número estoy pensando entre 1 y %d? ", max); // Nos pregunta que numero ha
																						// pensado el ordenador entre 1
																						// y max que es el primer
																						// aleatorio
			...
			// recordemos que todavia estamos del while que se ejecutara mientras jugar es
			// Si
			System.out.println("Felicidades, has acertado. ¿Quieres jugar otra vez? (si/no)");
			jugar = input.next();

		} // Cerramos el while que se saldra de el cuando el usuario diga algo distinto a
			// si, a la pregunta de si quiere jugar
